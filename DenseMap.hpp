...
namespace MZ
{
    namespace DenseMap
    {
        static const uint32_t Build = 1017;

#if defined(DENSE_FIX1)
        static const std::string Optimization = "fix1";
#elif defined(DENSE_FIX2)
        static const std::string Optimization = "fix2";
#else
        static const std::string Optimization = "optimal";
#endif
...
#pragma pack(pop)
        ...
        public:
            ...
            std::span<TKey> GetKeysForCmp()
            {
                ...
                for (uint32_t i = 0; i < _Capacity; i++)
                {
                    ...
                    *ptr = *reinterpret_cast<TKey*>(_entries + i); ptr++;
                }
        ...
        template <typename TKey, typename TValue>
        class HashMap : public Core<TKey, TValue, KeyValueEntry<TKey, TValue>>
        {
        public:
            HashMap(int32_t size) : Core<TKey, TValue, KeyValueEntry<TKey, TValue>>(size) {}
        ...
        class HashIndex : public Core<TKey, uint32_t, KeyValueEntry<TKey, uint32_t>>
        {
        public:

            static const uint32_t KEY_EXIST_MASK = 0x80000000;

            HashIndex(int32_t size) : Core<TKey, uint32_t, KeyValueEntry<TKey, uint32_t>>(size) {}
        ...
        template <typename TKey>
        class HashTable : public Core<TKey, uint32_t, TKey>
        {
        public:

            HashTable(int32_t size) : Core<TKey, uint32_t, TKey>(size) {}
